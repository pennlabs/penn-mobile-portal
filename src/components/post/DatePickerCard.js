...
const DatePickerCard = ({ updateStartDate, updateEndDate }) => {
  ...
  const handleStartTime = (time, mode) => {
    ...
    if (!moment(formatted, 'h:mm A', true).isValid()) {
      //input is not in valid 12-hr format
      ...
      let newDate =
        ...
        ' ' +
        moment(formatted, 'h:mm A').format('HH:mm')
      setData({
        ...data,
        startMode: mode,
        startTime: time,
      ...
      <div className="columns" style={{ display: 'flex' }}>
        <div className="column">
          <DatePicker
            ...
            minDate={new Date().toISOString()} //start date must be after current day
        ...
        <div className="column">
          <TimePicker
            ...
            onChange={(e) =>
              handleStartTime(e.target.value, data.startTimeMode)
            }
            style={{ fontFamily: 'inherit', borderBottom: 'none' }}
          >
            <TimePickerSelect
      ...
      <div className="columns" style={{ display: 'flex' }}>
        <div className="column">
          <DatePicker
            dateFormat="m/d/Y"
            datePickerType="single"
            minDate={data.startDate.toISOString()} //end date must be after start date
        ...
        <div className="column">
          ...
          >
            <TimePickerSelect
              ...
              labelText="AMPM"
              onChange={(e) => handleEndTime(data.endTime, e.target.value)}
              style={{ fontFamily: 'inherit', borderBottom: 'none' }}
            >
              <SelectItem value="AM" text="AM" />
              <SelectItem value="PM" text="PM" />
